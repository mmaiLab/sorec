...
from mmdet.models.piza_modules.box_sequence_embedding import BoxSequenceEmbedding, BBoxSequenceInferenceModel
from pathlib import Path
from typing import Dict, Optional, Tuple, Union, List


@MODELS.register_module()
class DemoPizaAdapterGroundingDINO(GroundingDINO):
    def __init__(self,
                 trainable_keys: List[str] = [], 
                 freeze_keys: List[str] = [],
                 use_zoom_embed=False, 
                 use_bbox_history=False, 
                 separate_bbox_history_module_path=None, 
                 use_bbox_history_encoder_feat=False,
                 use_2step_prediction=True,
                 use_multistep_prediction=True, 
                 adapter_type='adapter',
                 use_img_feat_adapter=True, 
                 use_text_feat_adapter=False,
        ...
        self.use_bbox_history = use_bbox_history
        self.separate_bbox_history_module_path = separate_bbox_history_module_path
        self.use_bbox_history_encoder_feat = use_bbox_history_encoder_feat
        self.use_2step_prediction = use_2step_prediction
        self.use_multistep_prediction = use_multistep_prediction
        self.adapter_type = adapter_type
        self.use_img_feat_adapter = use_img_feat_adapter
        self.use_text_feat_adapter = use_text_feat_adapter
        ...
        self.freeze_backbone_norm = freeze_backbone_norm

        
        super().__init__(*args, **kwargs)
        if self.use_zoom_embed or (self.use_bbox_history and (not self.use_bbox_history_encoder_feat)):
            self.zoom_embedding = TimeEmbedding(emb_in_ch, emb_out_ch)
                
        if self.separate_bbox_history_module_path is not None:
            self.bbox2eos_zoom_model = BBoxSequenceInferenceModel()

    def _init_layers(self) -> None:
        """Initialize layers except for backbone, neck and bbox_head."""
        self.positional_encoding = SinePositionalEncoding(
            **self.positional_encoding)
        self.encoder = GroundingDinoTransformerEncoderWithAdapter(
    ...
    def forward_transformer(
        ...
        eos, zoomVal, feat = self.bbox2eos_zoom_model(bbox_history)
        zoom_emb = self.zoom_embedding(zoomVal)
        encoder_outputs_dict = self.forward_encoder(
            **encoder_inputs_dict, text_dict=text_dict, zoom_emb=zoom_emb)
        encoder_outputs_dict = self.forward_encoder(
            **encoder_inputs_dict, text_dict=text_dict, zoom_emb=zoom_emb)
        ...
        if (batch_data_samples[0].get('dataset_mode') != 'VG') and (self.separate_bbox_history_module_path is not None):
    ...
    def inference_with_crop(self, batch_inputs, batch_data_samples, t=None, rescale=None, crop_box=None, bbox_history=None):
        pipeline_inputs = []
        ...
        for i, data_samples in enumerate(batch_data_samples):
            ...
            if self.separate_bbox_history_module_path is not None:
                head_inputs_dict, eos = self.forward_transformer(
                    copy.deepcopy(visual_feats), text_dict, batch_data_samples, bbox_history=bbox_history)
                if eos.item() == True:
    ...
    def predict(self, batch_inputs, batch_data_samples, rescale: bool = True):
        ...
        for pred_step in range(MAX_ZOOM):
            ...
            if pred_step == 0:
                _results_list, _entities, _is_rec_tasks, eos = self.inference_with_crop(batch_inputs, batch_data_samples, 0, rescale=rescale, crop_box=None, bbox_history=bbox_history_tensor)
            else:
                _results_list, _entities, _is_rec_tasks, eos = self.inference_with_crop(batch_inputs, batch_data_samples, 0, rescale=rescale, crop_box=[crop_box], bbox_history=bbox_history_tensor)
            if eos.item():
                break
            if pred_step != 0:
    ...
    def get_tokens_positive_and_prompts(
        ...
        return None, caption_string, None, entities